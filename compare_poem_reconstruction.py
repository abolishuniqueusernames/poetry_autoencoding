...
def compare_poem_reconstruction(model: RNNAutoencoder, dataset: AutoencoderDataset, 
    ...
    poem_info = poems[metadata['poem_index']]
    print(f"📖 Original Poem: \"{poem_info['title']}\" by {poem_info['author']}")
    print(f"📊 Chunk: {metadata['chunk_index'] + 1}/{metadata['total_chunks']} (tokens {metadata['start_token']}-{metadata['end_token']})")
    
    # Extract original text chunk
    original_text = poem_info['content']
    words = original_text.split()
    chunk_words = words[metadata['start_token']:metadata['end_token']]
