...
def load_model_checkpoint(checkpoint_path, hidden_size):
    ...
    if is_new_architecture:
        ...
        if hasattr(autoencoder.decoder, 'teacher_forcing_projection'):
            # Remove teacher forcing projection and make decoder RNN take 300D input
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_loader):
            ...
            if hasattr(model.decoder, 'teacher_forcing_projection'):
                ...
                # Old architecture - decoder expects embeddings directly
                # Use the full autoencoder forward pass instead
                reconstructed, _ = model(input_sequences, attention_mask)
...
def analyze_bottleneck_space(baseline_results, scaled_results):
    ...
    for name, bottlenecks in [("Baseline", baseline_bottlenecks), ("Scaled", scaled_bottlenecks)]:
        ...
        print(f"      Effective dimensions: {effective_dims}/16")
...
def create_comparison_visualization(baseline_results, scaled_results):
    ...
    baseline_pca = pca.fit_transform(baseline_results['bottlenecks'])
    ...
    scaled_pca = pca.fit_transform(scaled_results['bottlenecks'])
